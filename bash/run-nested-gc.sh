...
for run in base pert-{0..11}; do
    for scenario in 1 2 3 4; do
        ...
        run_dir="${dir}/nested/scenario-${scenario}/${run}"
        mkdir -p "${run_dir}"
        cd "${run_dir}"

        # Compile GCClassic 14.3.1 for CH4/MERRA2/0.5x0.625/47 L
        ...
        sed -i -e "s|UseTotalPriorEmis      :       false|UseTotalPriorEmis      :       true|g" \
            ...
            -e "s|../../prior_run/OutputDir/HEMCO_sa_diagnostics.\$YYYY\$MM\$DD0000.nc|${dir}/emis/modified_emis/nested-${emis_type}-${run}.nc|g" \
        ...
        # Point to correct BCs (optionally of 1 ppb)
        if [ "$scenario" -eq 1 ] || [ "$scenario" -eq 2 ]; then
            sed -i -e "s|\$ROOT/SAMPLE_BCs/v2021-07/CH4/|${dir}/bcs/one_ppb/|g" HEMCO_Config.rc
        elif [ "$scenario" -eq 3 ] || [ "$scenario" -eq 4 ]; then
            sed -i -e "s|\$ROOT/SAMPLE_BCs/v2021-07/CH4/|${dir}/bcs/normal/|g" HEMCO_Config.rc
        fi
